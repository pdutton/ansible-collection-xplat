...
class ActionModule(ActionBase):
    """Cross-platform hostname action plugin.

    Abstracts differences between ansible.builtin.hostname (Linux/Mac) and
    ansible.windows.win_hostname (Windows) by detecting the target OS and
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        if 'name' not in self._task.args:
            raise AnsibleActionFail('name is required for pdutton.xplat.hostname')
        ...
        if os_family == 'windows' or system == 'win32nt':
            module_name = 'ansible.windows.win_hostname'
        else:
            module_name = 'ansible.builtin.hostname'
