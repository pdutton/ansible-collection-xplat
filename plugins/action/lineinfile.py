...
class ActionModule(ActionBase):
    """Cross-platform lineinfile action plugin.

    Abstracts differences between ansible.builtin.lineinfile (Linux/Mac) and
    ansible.windows.win_lineinfile (Windows) by detecting the target OS and
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        if 'path' not in self._task.args:
            raise AnsibleActionFail('path is required for pdutton.xplat.lineinfile')
        ...
        if os_family == 'windows' or system == 'win32nt':
            module_name = 'ansible.windows.win_lineinfile'
        else:
