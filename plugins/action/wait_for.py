...
class ActionModule(ActionBase):
    """Cross-platform wait_for action plugin.

    Abstracts differences between ansible.builtin.wait_for (Linux/Mac) and
    ansible.windows.win_wait_for (Windows) by detecting the target OS and
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        if os_family == 'windows' or system == 'win32nt':
            module_name = 'ansible.windows.win_wait_for'
        else:
            module_name = 'ansible.builtin.wait_for'
