...
class ActionModule(ActionBase):
    """Cross-platform find action plugin.

    Abstracts differences between ansible.builtin.find (Linux/Mac) and
    ansible.windows.win_find (Windows) by detecting the target OS and
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        if 'paths' not in self._task.args:
            raise AnsibleActionFail('paths is required for pdutton.xplat.find')
