...
class ActionModule(ActionBase):
    """Cross-platform stat action plugin.

    Abstracts differences between ansible.builtin.stat (Linux/Mac) and
    ansible.windows.win_stat (Windows) by detecting the target OS and
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        if 'path' not in self._task.args:
            raise AnsibleActionFail('path is required for pdutton.xplat.stat')
        ...
        if os_family == 'windows' or system == 'win32nt':
            # Use Windows stat module
            module_name = 'ansible.windows.win_stat'
        else:
            # Use built-in stat module for Linux, macOS, and other Unix variants
