...
class ActionModule(ActionBase):
    """Cross-platform uri action plugin.

    Abstracts differences between ansible.builtin.uri (Linux/Mac) and
    ansible.windows.win_uri (Windows) by detecting the target OS and
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        if 'url' not in self._task.args:
            raise AnsibleActionFail('url is required for pdutton.xplat.uri')
        ...
        if os_family == 'windows' or system == 'win32nt':
            module_name = 'ansible.windows.win_uri'
        else:
