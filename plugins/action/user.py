...
class ActionModule(ActionBase):
    """Cross-platform user action plugin.

    Abstracts differences between ansible.builtin.user (Linux/Mac) and
    ansible.windows.win_user (Windows) by detecting the target OS and
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        if 'name' not in self._task.args:
            raise AnsibleActionFail('name is required for pdutton.xplat.user')
        ...
        if os_family == 'windows' or system == 'win32nt':
            module_name = 'ansible.windows.win_user'
        else:
